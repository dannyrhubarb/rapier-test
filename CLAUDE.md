...
- **Ship rendering**: the hull is the const `SHIP_TRIS` — 41 triangles **extracted from the original Flash ship** (see below) — drawn in local ship space (`+Y` = nose/forward, origin = hull centroid, full height ≈ 0.95 world units). Each facet's silver brightness is derived from its centroid height (nose lit → base shaded). A small cyan canopy triangle and a two-triangle orange/yellow thruster flame (scaled by `glow`, hidden when `glow ≤ 0.02`, drawn first so it sits behind the hull) complete it. All geometry goes through the `rot(lx, ly)` closure → `w2s`.
- **Origin of the ship mesh**: the geometry is the real player ship from the original Flash game. The published SWF (`completeHS8replay.swf`, a `CWS` zlib-compressed SWF) was decompressed and its tags parsed; the ship is `DefineShape4` **character id 41** (`mcSpaceship`), a silver lander (`#999999`/`#CCCCCC`). Its vector contours were rasterised, the outer silhouette traced and RDP-simplified to a 43-pt polygon, then ear-clip triangulated to `SHIP_TRIS`. (The source `.fla` is an OLE compound doc whose binary edge format is undocumented; the SWF shape format **is** documented, so extraction was done from the SWF.) Regeneration scripts live only in scratch (`/tmp`), not the repo.
...
## Git workflow
- Development branch: `claude/walls-obstacles-appearance-qj1rpp` (current); previous: `claude/cave-polygon-obstacles-8lxhh2`
