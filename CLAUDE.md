...
- **Ship rendering**: drawn as a set of `draw_triangle` calls in local ship-space coordinates (y+ = nose/thrust direction, origin = center). Triangles: nose (hull_hi), upper/mid/lower body bands (hull_mid/hull_lo), engine bell (engine_c), swept-back delta **red fins** with shadow inset (wing_c/wing_lo), cyan canopy (cockpit_c). Thruster flame (two overlapping triangles in orange/yellow) is drawn first (behind hull) and scales with `glow` (hidden when `glow ≤ 0.02`). All geometry uses the `rot(lx, ly)` closure to rotate into world space then `w2s` to screen.
- **Origin of the design**: the silver-grey + red palette is taken from the original Flash project's `mcSpaceship` symbol (Symbol 15 in the `.fla` OLE compound doc), which used fills red `#FF0000` + light grey `#CCCCCC`. The `.fla` binary edge format (variable-length i16-twip deltas gated by undocumented control bytes) was only partially decoded, so the silhouette is a faithful hand-built lander rather than a byte-exact extraction.
