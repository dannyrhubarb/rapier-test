...
**RCS / attitude thrusters** (cosmetic particles, `kind 1/2`): bottom nozzles flanking the main booster vent sideways. Turning **left** → right nozzle at scaled-local `(0.18, −0.64)` fires gas out `+X`; turning **right** → left nozzle at `(−0.18, −0.64)` fires gas out `−X`. Emission coords are in **scaled world units** — `lp()`/`ld()` do **not** apply `SHIP_SCALE` (only the render-time `rot` closure does), so don't multiply these by `SHIP_SCALE` (an earlier bug double-scaled them to ±0.60 and spawned the puffs outside the hull).
